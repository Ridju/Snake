class Snake { 
    ...
    getCollision(canvas){
        ...
        const result = this.elements.filter((element) => {
            return element.x === this.elements[0].x && element.y === this.elements[0].y
        })

        var collisionDetected = result.length > 1;

        //Snake hit border
        if(this.elements[0].x < 0 || this.elements[0].x > canvas.width || 
            this.elements[0].y < 0 || this.elements[0].y > canvas.height) { 
    ...
    move(dx, dy, food) { 
        ...
        const has_eaten = this.elements[0].x === food.x && this.elements[0].y === food.y;
        if(has_eaten){ 
            food.genNew();
            return true;
        } else { 
